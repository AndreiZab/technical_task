...
public:
    ...
    void AddValue(uint32_t value) {
        ...
        // можно, если он не сильно отклоняет avg и присутствие последовательность во времени приема
            ...
            last->avg = (last->avg * (float)last->dur_s + rx_val) / ((float)last->dur_s + 1.0f);
            last->dur_s++;
        }
        ...
        if (large.size() > VEC_TARGET_SIZE) {
            ...
            for (auto it = begin (large); it != end (large); it++) {
                src_sum.avg += it->avg * (float)it->dur_s / (float)full_duration;
            }
            src_sum.start_s = large.front().start_s;
            src_sum.dur_s = full_duration;

            // TODO need overflow protect
            ultimate.avg = (((float)src_sum.dur_s * src_sum.avg)
                            + ((float)ultimate.dur_s * ultimate.avg))
                           / (float)(src_sum.dur_s + ultimate.dur_s);
...
private:
    ...
    void sum_elem(const std::vector<avg_chunk_t>& vec,
        ...
        if (!vec.empty()) {
            ...
            while ((i < vec.size()) &&
                ...
                (*avg_spd) += (vec[i].avg / (float)avg_time_s * (float)dur);
                (*sum_dur) += dur;
                i--;
            }
        }
    ...
    float calc_avg_spd(const time_t cur_time, const uint32_t avg_time_s) {
        ...
        if ((ultimate.start_s + ultimate.dur_s) > (cur_time - avg_time_s)) {
            avg_spd += (ultimate.avg / (float)avg_time_s * (float)(avg_time_s - sum_dur));
        }
    ...
    void average_and_transfer_data(std::vector<avg_chunk_t>& src, std::vector<avg_chunk_t>& dst) {
        ...
        for (auto it = begin (src); it != end (src); it++) {
            src_sum.avg += it->avg * (float)it->dur_s / (float)full_src_duration;
        }
        ...
        if ((!dst.empty()) &&
            ...
            // TODO need overflow protect
            dst.back().avg = (((float)src_sum.dur_s * src_sum.avg)
                            + ((float)dst.back().dur_s * dst.back().avg))
                            /  (float)(src_sum.dur_s + dst.back().dur_s);
